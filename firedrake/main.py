...
# 2D mesh
mesh = RectangleMesh(mx, my, L_1, L_2, quadrilateral=True)
x, y = SpatialCoordinate(mesh)

# # 3D mesh
# # First: Create a structured 2D quadrilateral mesh
# base_mesh = RectangleMesh(mx, my, L_1, L_2, quadrilateral=True)
# # Then: Extrude the mesh in the z-direction to form a cube (hexahedral mesh)
# mesh = ExtrudedMesh(base_mesh, layers=mz, layer_height=L_3/mz)
...
# Function spaces
V     = VectorFunctionSpace(mesh, "CG", 2)  # Quadratic for displacement
W     = FunctionSpace(mesh, "CG", 1)        # Linear for pore pressure
X     = FunctionSpace(mesh, "CG", 1)        # Linear for fracture
Z     = V * W                               # Mixed function space for u, p
...
z.sub(1).interpolate(p_0)
z_old.assign(z)

# Initial crack(s)
phi_0 = Function(X).interpolate(conditional(And(x <= (L_1+a_0)/2, And(x >= (L_1-a_0)/2, And(y < 0.5*L_2+min_dy, y > 0.5*L_2-min_dy))), 1.0, 0.0))
phi.assign(phi_0)
...
outfile = VTKFile("solution.pvd", "w")
outfile.write(project(u, V_out, name="u"), project(p, W, name="p"), project(phi, X, name="phi"), time=0.0)
...
for i in range(n_steps):
    ...
    # Prevent healing
    phi.interpolate(max_value(phi_old, phi))

    # Boundedness
    phi.interpolate(min_value(max_value(phi, 0.0), 1.0))
    ...
    if (i+1) % output_interval == 0:
        outfile.write(project(u, V_out, name="u"), project(p, W, name="p"), project(phi, X, name="phi"), time=dt*(i+1))

    # Results Monitoring
    rtime.append(float(dt*(i+1)))
    p_c_now = float(p_h.at((L_1/2, L_2/2)))
...
np.savetxt("p_c.txt", np.column_stack([rtime, p_c]), header="time [s]    p_c [MPa]", fmt="%.8e")